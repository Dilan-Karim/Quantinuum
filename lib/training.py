...
"""
This module contains the TrainerWrapper class for training a quantum model using PyTorch.
"""
...
class TrainerWrapper:
    ...
    def plot(self):
        ...
        This method creates a 5x2 grid of subplots to visualize the training and validation metrics.
        The metrics include loss, accuracy, recall, precision, and F1 score.
        The plots are saved as a PDF file in the './output/plots/' directory.
        ...
        fig, axs = plt.subplots(5, 2,
                                ...
                                figsize=(12,25))  # Adjust to a 5x2 grid layout
        ...
        axs[1, 0].plot(range_, self.trainer.train_eval_results['acc'],
                       label='train', color='tab:purple')
        axs[1, 0].plot(range_, self.trainer.val_eval_results['acc'],
                       label='val', color='tab:orange')

        axs[0, 1].plot(range_, self.trainer_random.train_epoch_costs,
                       label='train', color='tab:purple')
        axs[0, 1].plot(range_, self.trainer_random.val_costs,
                       label='val', color='tab:orange')

        axs[1, 1].plot(range_, self.trainer_random.train_eval_results['acc'],
                       label='train', color='tab:purple')
        axs[1, 1].plot(range_, self.trainer_random.val_eval_results['acc'],
                       label='val', color='tab:orange')

        # Plot Recall, Precision, and F1 Score (new metrics)
        for metric, row in zip(['recall', 'precision', 'f1'], range(2, 5)):
            axs[row, 0].plot(range_, self.trainer.train_eval_results[metric],
                             label='train', color='tab:purple')
            axs[row, 0].plot(range_, self.trainer.val_eval_results[metric],
                             label='val', color='tab:orange')

            axs[row, 1].plot(range_, self.trainer_random.train_eval_results[metric],
                             label='train', color='tab:purple')
            axs[row, 1].plot(range_, self.trainer_random.val_eval_results[metric],
                             label='val', color='tab:orange')
